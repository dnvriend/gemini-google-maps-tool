...
typecheck: ## Run type checking with mypy
	uv run mypy gemini_google_maps_tool

test: ## Run tests
	uv run pytest tests/

security-bandit: ## Run bandit security linter
	uv run bandit -r gemini_google_maps_tool -c pyproject.toml
...
run: ## Run gemini-google-maps-tool (usage: make run ARGS="...")
...
uninstall-global: ## Uninstall global installation
	uv tool uninstall gemini-google-maps-tool
